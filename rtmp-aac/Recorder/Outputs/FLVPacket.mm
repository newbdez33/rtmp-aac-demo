...
#pragma mark - Metadata start
    ...
    *enc++ = AMF_OBJECT_END;                                                // Mark the end of the array                                    0x09
    ...
    put_be24(&metaDataSizePos, (unsigned int)dataSize);
    
    //FLVLog(@"Checkpoint #%i\n\n  enc = %p\n  *enc = %p\n\n  buffer = %p\n  *buffer = %p\n\n  metaDataSizePos = %p\n  *metaDataSizePos = %p\n\n  dataSize = %llu", kNumCheckpoint++, enc, (void*)*enc, buffer, (void*)*buffer, metaDataSizePos, (void*)*metaDataSizePos, dataSize);
    
    *buffer = (uint8_t*)enc;
    
    put_be32(buffer, (unsigned int)dataSize + 11);                                    // Write the previous tag size                                  0xAA
...
#pragma mark - Audio Data Start
    ...
    *buffer = (uint8_t*)sizePos;
    ...
    put_be24(buffer, (unsigned int)dataSize);
    
    *buffer = ref;
    
    put_be32(buffer, (unsigned int)dataSize + 11);
...
#pragma mark - Video Data Start
    ...
    avcCHeader header((const BYTE*)[avcC bytes], (unsigned int)[avcC length]);        // Wrapper for avcC data
    ...
    *buffer = (uint8_t*)sizePos;
    ...
    put_be24(buffer, (unsigned int)dataSize);                                         // Write the size of the SPS/PPS data at avcCSizePos            0xED
    
    *buffer = ref;
    
    put_be32(buffer, (unsigned int)dataSize + 11);                                    // Write previous tag size                                      0xE8
    ...
    *buffer = pStart;                                                   // Reset the pointer back to pStart
    ...
    return (unsigned int)dataSize;                                                    // Return the size of the data
}
...
- (int)writeAvcCHeader:(NSData*)avcC toBuffer:(uint8_t**)buffer {
    ...
    avcCHeader header((const BYTE*)[avcC bytes], (unsigned int)[avcC length]);
